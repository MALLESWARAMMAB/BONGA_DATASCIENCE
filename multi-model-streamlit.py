...
MODEL_CONFIGS = {
    ...
    "LR Model": {
        "file": "Logistic_Reg.pkl",
        #"image": "model2_image.png",
        "features": [
            ...
            {"name": "Operating Risk", "min": 0.0, "max": 2.0, "step": 0.5}
        ]
    },
    "RF Model": {
        "file": "random_forest_model.pkl",
        #"image": "model3_image.png",
        "features": [
            {"name": "Financial Flexibility", "min": 0.0, "max": 1.0, "step": 0.5},
            {"name": "Credibility", "min": 0.0, "max": 1.0, "step": 0.5},
            {"name": "Competitiveness", "min": 0.0, "max": 1.0, "step": 0.5}
        ]
    },
...
#with col1:
    #image_path = MODEL_CONFIGS[selected_model]["image"]
    #if os.path.exists(image_path):
        #image = load_image(image_path)
        #if image:
            #st.image(image, caption=f"{selected_model} Visualization")
    #else:
...
if model:
    ...
    if st.button("Predict Bankruptcy"):
        ...
        if prediction is not None:
            ...
            with result_col1:
                result_text = "May Go Bankrupt" if prediction == 0 else "May Not Go Bankrupt"
